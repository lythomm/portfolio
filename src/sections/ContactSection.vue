...
const animContainerRef = ref(null)
...
onMounted(() => {
  ...
  // Fade-in content container as we scroll to the bottom
  scrollCtx = gsap.context(() => {
    gsap.fromTo('.contact-anim-content',
      { opacity: 0 },
      {
        opacity: 1,
        ease: 'power3.in',
...
<template>
  <footer id="contact" ref="animContainerRef" class="bg-[var(--color-primary)] pt-16 md:pt-12 pb-12 md:pb-8 relative overflow-hidden md:h-screen md:flex md:flex-col md:justify-between">
    <!-- Mesh Gradient Background Shader -->
    <div ref="shaderCanvasContainer" class="footer-shader-container"></div>
    
    <!-- Anim content wrapper -->
    <div class="contact-anim-content w-full flex flex-col justify-between flex-grow z-10 opacity-0">
